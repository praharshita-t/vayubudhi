...
def load_and_prepare_data():
    ...
    # Synthesize targets for training
    # 1. Forecast Target: Future AQI
    # We simulate a 24h future AQI
    df['target_aqi_24h'] = df['pm25'] * 1.5 + np.random.normal(10, 15, len(df))
    df['target_aqi_24h'] = df['target_aqi_24h'].clip(lower=0)
    y_reg = df['target_aqi_24h']
    
    # 2. Classification Target: Pollution Source
    # Let's create a synthetic heuristic so the model learns something
    sources = []
...
def train_models():
    X, y_reg, y_clf = load_and_prepare_data()
    
    X_train_r, X_test_r, y_train_r, y_test_r = train_test_split(X, y_reg, test_size=0.2, random_state=42)
    X_train_c, X_test_c, y_train_c, y_test_c = train_test_split(X, y_clf, test_size=0.2, random_state=42)
    
    print("Training XGBoost Forecaster with MAPIE...")
    ...
    print("Saving models...")
    os.makedirs(ML_DATA_DIR, exist_ok=True)
    joblib.dump(xgb, os.path.join(ML_DATA_DIR, 'forecaster.pkl'))
    joblib.dump(rfc, os.path.join(ML_DATA_DIR, 'classifier.pkl'))
    
    print("\nVerifying model predictions on a sample (matching API requirement):")
    sample_df = X.head(1)
    
    pred_point = xgb.predict(sample_df)[0]
    pred_class = rfc.predict(sample_df)[0]
